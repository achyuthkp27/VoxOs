...
enum VoxOSRefineInferenceError: LocalizedError {
    case unavailable
    case modelNotLoaded
    case emptyOutput
    ...
    var errorDescription: String? {
        switch self {
        case .unavailable:
            return "VoxOS Refine requires Apple silicon."
        case .modelNotLoaded:
            return "VoxOS Refine could not load the selected model."
        case .emptyOutput:
            return "VoxOS Refine returned an empty response."
        }
    }
}

actor VoxOSRefineInferenceEngine {
    #if arch(arm64)
    ...
    func prepare(
        ...
        #else
            throw VoxOSRefineInferenceError.unavailable
        #endif
    }
    ...
    func enhance(
        ...
        systemPrompt: String
            ...
            guard let modelContainer else {
                throw VoxOSRefineInferenceError.modelNotLoaded
            }
            ...
            guard !output.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                throw VoxOSRefineInferenceError.emptyOutput
            }
