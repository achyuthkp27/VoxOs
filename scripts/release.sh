...
PROJECT_PATH="$REPO_ROOT/VoxOS.xcodeproj"
SCHEME="VoxOS"
...
WHISPER_FRAMEWORK="${VOXOS_WHISPER_FRAMEWORK:-$HOME/VoxOS-Dependencies/whisper.cpp/build-apple/whisper.xcframework}"

DEVELOPER_IDENTITY="${VOXOS_DEVELOPER_IDENTITY:-Developer ID Application: Prakash Joshi (V6J6A3VWY2)}"
NOTARY_PROFILE="${VOXOS_NOTARY_PROFILE:-VoxOS-Notarization}"
SPARKLE_ACCOUNT="${VOXOS_SPARKLE_ACCOUNT:-VoxOS}"
RELEASE_BASE_URL="${VOXOS_RELEASE_BASE_URL:-https://github.com/Beingpax/VoxOS/releases/download}"
EXPECTED_FEED_URL="https://beingpax.github.io/VoxOS/appcast.xml"
EXPECTED_BUNDLE_ID="com.achyuthkp.VoxOS"
EXPECTED_MINIMUM_SYSTEM_VERSION="14.4"

XCODE_DEVELOPER_DIR="${VOXOS_XCODE_DEVELOPER_DIR:-${DEVELOPER_DIR:-}}"
...
RELEASE_OUTPUT_ROOT="${VOXOS_RELEASE_OUTPUT_ROOT:-$HOME/Downloads/VoxOS Builds}"
OUTPUT_DIR="$RELEASE_OUTPUT_ROOT/VoxOS-$RELEASE_TIMESTAMP"
...
Builds, signs, notarizes, packages, validates, and creates VoxOS's Appcast.
The script never commits, pushes, tags, or creates a GitHub release.

Options:
  --app <VoxOS.app>        Use an existing exported app instead of archiving.
...
Examples:
  ...
  scripts/release.sh --app /path/to/VoxOS.app \
...
find_sparkle_tools() {
    ...
    candidate="$(find "$HOME/Library/Developer/Xcode/DerivedData" \
        -path '*/VoxOS-*/SourcePackages/artifacts/sparkle/Sparkle/bin/generate_appcast' \
    ...
    [[ -n "$candidate" ]] || fail "Sparkle tools not found. Resolve VoxOS's Swift packages first."
...
APP_PATH="$EXPORT_DIR/VoxOS.app"
INFO_PLIST="$APP_PATH/Contents/Info.plist"
[[ -d "$APP_PATH" ]] || fail "Exported VoxOS.app not found"
...
DOWNLOAD_URL="$RELEASE_BASE_URL/$RELEASE_TAG/VoxOS.dmg"
...
ditto "$APP_PATH" "$DMG_SOURCE_DIR/VoxOS.app"
...
create-dmg \
    ...
    --icon "VoxOS.app" "$DMG_APP_X" "$DMG_APP_Y" \
    --hide-extension "VoxOS.app" \
...
DMG_MOUNT_DIR="$(mktemp -d /tmp/voxos-release-mount.XXXXXX)"
hdiutil attach -readonly -nobrowse -mountpoint "$DMG_MOUNT_DIR" "$DMG_PATH"
DMG_MOUNTED=1

MOUNTED_APP="$DMG_MOUNT_DIR/VoxOS.app"
[[ -d "$MOUNTED_APP" ]] || fail "VoxOS.app is missing from the DMG"
...
ditto "$DMG_PATH" "$APPCAST_WORK_DIR/VoxOS.dmg"
cp "$NOTES_PATH" "$APPCAST_WORK_DIR/VoxOS.$NOTES_EXTENSION"
