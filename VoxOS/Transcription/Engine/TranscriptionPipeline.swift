...
class TranscriptionPipeline {
    ...
    private let logger = Logger(subsystem: "com.achyuthkp.voxos", category: "TranscriptionPipeline")
    ...
    func run(
        ...
        do {
            ...
            if !assistant.isFollowUp {
                ...
                let isConfiguredForGate =
                    resolvedEnhancementConfiguration.map { self.enhancementService?.isConfigured(for: $0) == true }
                    ?? false
                logger.debug(
                    "pipeline gate: outputMode=\(String(describing: resolvedOutputConfiguration.outputMode), privacy: .public) enhancementEnabled=\(String(describing: resolvedEnhancementConfiguration?.isEnabled), privacy: .public) provider=\(String(describing: resolvedEnhancementConfiguration?.provider), privacy: .public) prompt=\(String(describing: resolvedEnhancementConfiguration?.prompt?.title), privacy: .public) isConfigured=\(isConfiguredForGate, privacy: .public) shouldRespond=\(shouldRespondInRecorder, privacy: .public)"
                )
