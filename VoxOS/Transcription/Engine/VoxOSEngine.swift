...
@MainActor
class VoxOSEngine: NSObject, ObservableObject {
    ...
    private var voxOSRefinePreparationTask: Task<Void, Never>?
    ...
    let logger = Logger(subsystem: "com.achyuthkp.voxos", category: "VoxOSEngine")
                ...
                if granted {
                    Task { @MainActor [self] in
                        ...
                        do {
                            ...
                            self.scheduleVoxOSRefinePreparation(for: startID)
    ...
    private func scheduleVoxOSRefinePreparation(for recordingStartID: UUID) {
        voxOSRefinePreparationTask?.cancel()

        voxOSRefinePreparationTask = Task { @MainActor [weak self] in
            guard let self,
                self.recordingState == .recording,
            ...
            guard initialConfiguration.isEnabled,
                initialConfiguration.provider == .voxOSRefine
            else {
                return
            }
            ...
            await aiService.voxOSRefineService.keepPreparedModelWarmForRecording()
            ...
            guard configuration.isEnabled, configuration.provider == .voxOSRefine else {
                return
            }

            await aiService.voxOSRefineService.prepareForRecording()
        }
    }
    ...
    private func finishRecorderSession() async {
        let preparationTask = voxOSRefinePreparationTask
        voxOSRefinePreparationTask = nil
