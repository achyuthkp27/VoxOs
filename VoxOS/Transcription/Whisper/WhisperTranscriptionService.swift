...
class WhisperTranscriptionService: TranscriptionService {
    ...
    private let logger = Logger(subsystem: "com.achyuthkp.voxos", category: "WhisperTranscriptionService")
        ...
        -> String
    {
        guard model.provider == .whisper else {
            throw VoxOSEngineError.modelLoadFailed
        }
    ...
    private func readAudioSamples(_ url: URL) throws -> [Float] {
        ...
        let headerLength = 44
        let sampleBytes = data.count - headerLength
        guard sampleBytes > 0 else { return [] }
        // A truncated file can have an odd length; the last lone byte is not a sample.
        let floats = stride(from: headerLength, to: headerLength + (sampleBytes & ~1), by: 2).map {
