...
struct ModeConfigFormView: View {
            ...
            Toggle("AI Enhancement", isOn: $draft.isAIEnhancementEnabled)
                .onChange(of: draft.isAIEnhancementEnabled) { _, newValue in
                    if newValue {
                        ...
                        if configuredSelectedAIProvider != .voxOSRefine,
    ...
    private var canRespond: Bool {
        draft.isAIEnhancementEnabled
            ...
            && configuredSelectedAIProvider != .voxOSRefine
    }
    ...
    private func applyVoxOSRefineRulesIfNeeded() {
        guard configuredSelectedAIProvider == .voxOSRefine else { return }
        applyVoxOSRefineRules()
    }

    private func applyVoxOSRefineRules() {
        draft.selectedAIModel = VoxOSRefineService.modelName
        applyOutputRules()
    }
    ...
    private var customCommandControls: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 6) {
                ...
                InfoTip(
                    LocalizedStringKey(
                        "Runs locally with your user permissions. The final transcript is sent on stdin and exposed as VOXOS_TRANSCRIPT."
                    ))
                Spacer()
                Menu {
