...
struct ModeView: View {
    ...
    private var headerControls: some View {
        HStack(spacing: 8) {
            if !StarterModeFactory.isInstalled(kind: .agent) {
                addAgentModeButton
            }
            addModeButton
            settingsButton
    ...
    private var addAgentModeButton: some View {
        AppIconButton(
            systemName: "wand.and.stars",
            help: "Add the Agent mode, fully wired up with a working AI provider and a Control+Option shortcut"
        ) {
            StarterModeFactory.quickInstallAgent(snapshot: modeWarmupStore.snapshot)
        }
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            AppScreenHeader(
                title: "Modes",
                infoMessage: "Modes help you set up VoxOS for different writing tasks, workflows, and scenarios.",
