...
enum AIProvider: String, CaseIterable {
    ...
    var requiresAPIKey: Bool {
        switch self {
        case .voxOSRefine, .ollama, .localCLI:
            return false
        default:
            return true
        }
...
class AIService: ObservableObject {
    ...
    @Published var selectedProvider: AIProvider {
        didSet {
            ...
            if selectedProvider.requiresAPIKey {
                ...
                } else if selectedProvider == .voxOSRefine {
                    self.isAPIKeyValid = voxOSRefineService.isAvailableInModes
                } else {
                    self.isAPIKeyValid = true
                }
    ...
    var connectedProviders: [AIProvider] {
        AIProvider.allCases.filter { provider in
            ...
            } else if provider == .voxOSRefine {
                return voxOSRefineService.isAvailableInModes
    ...
    var currentModel: String {
        if selectedProvider == .voxOSRefine {
    ...
    init() {
        ...
        voxOSRefineObserver = voxOSRefineService.objectWillChange.sink { [weak self] _ in
            DispatchQueue.main.async {
                guard let self else { return }

                if self.selectedProvider == .voxOSRefine {
                    let isAvailable = self.voxOSRefineService.isAvailableInModes
    ...
    private func reloadSelectedProviderConfiguration() {
        ...
        if selectedProvider.requiresAPIKey {
            ...
            if selectedProvider == .localCLI {
                ...
                isAPIKeyValid = voxOSRefineService.isAvailableInModes
            } else {
                isAPIKeyValid = true
            }
        }
    ...
    func selectModel(_ model: String, for provider: AIProvider) {
        ...
        let resolvedModel = provider == .voxOSRefine ? provider.defaultModel : model
    ...
    func enhanceWithVoxOSRefine(transcript: String) async throws -> String {
        try await voxOSRefineService.enhance(transcript: transcript)
    }
