...
private final class VoxOSRefineXPCReply<Value>: @unchecked Sendable {
...
private final class VoxOSRefineXPCCancellationHandle: @unchecked Sendable {
...
private enum VoxOSRefineXPCOperation {
    case prepare
    case enhance
}

private enum VoxOSRefineXPCOperationWaiter {
...
actor VoxOSRefineXPCClient {
    ...
    func prepare(modelDirectory: URL, systemPrompt: String) async throws {
        ...
        let request = VoxOSRefinePrepareRequest(
            requestID: UUID(),
        ...
        let cancellationHandle = VoxOSRefineXPCCancellationHandle(activeConnection)

        do {
            try await withTaskCancellationHandler {
                try await withCheckedThrowingContinuation { continuation in
                    let reply = VoxOSRefineXPCReply<Void>(continuation)
                    guard
                        let proxy = activeConnection.remoteObjectProxyWithErrorHandler({
                            error in
                            reply.resolve(.failure(error))
                        }) as? VoxOSRefineXPCProtocol
                    else {
                        reply.resolve(
                            .failure(
                                makeVoxOSRefineXPCError(
                                    .connectionFailed,
                                    description: "Could not create the VoxOS Refine XPC proxy."
                                )
                            )
                        )
                        return
    ...
    func enhance(
        ...
        let request = VoxOSRefineEnhanceRequest(
            requestID: UUID(),
            modelDirectoryPath: modelDirectory.path,
            systemPrompt: systemPrompt,
        ...
        do {
            let responseData: Data = try await withTaskCancellationHandler {
                try await withCheckedThrowingContinuation { continuation in
                    let reply = VoxOSRefineXPCReply<Data>(continuation)
                    guard
                        let proxy = activeConnection.remoteObjectProxyWithErrorHandler({
                            error in
                            reply.resolve(.failure(error))
                            ...
                            reply.resolve(
                                .failure(
                                    makeVoxOSRefineXPCError(
                                        .invalidResponse,
                                        description:
                                            "VoxOS Refine returned an empty response."
                                    )
                                )
                            )
                        }
            ...
            let response = try JSONDecoder().decode(
                VoxOSRefineEnhanceResponse.self,
                from: responseData
            )
            guard response.requestID == request.requestID else {
                throw makeVoxOSRefineXPCError(
                    .invalidResponse,
                    description: "VoxOS Refine returned a mismatched response."
                )
            }
    ...
    private func connectionForRequest() -> NSXPCConnection {
        ...
        let identifier = UUID()
        let connection = NSXPCConnection(serviceName: voxOSRefineXPCServiceName)
    ...
    private func localizedError(
        _ error: Error,
        operation: VoxOSRefineXPCOperation
    ) -> Error {
        let nsError = error as NSError
        guard nsError.domain == voxOSRefineXPCErrorDomain,
            let code = VoxOSRefineXPCErrorCode(rawValue: nsError.code)
        else {
            return error
        }
        ...
        switch code {
        case .invalidRequest:
            description = String(localized: "VoxOS Refine received an invalid request.")
        case .inferenceFailed:
            switch operation {
            case .prepare:
                description = String(localized: "VoxOS Refine could not prepare the model.")
            case .enhance:
                description = String(
                    localized: "VoxOS Refine could not complete the enhancement."
                )
            }
        case .invalidResponse:
            description = String(localized: "VoxOS Refine returned an invalid response.")
        case .connectionFailed:
            description = String(localized: "Could not communicate with VoxOS Refine.")
        }

        return makeVoxOSRefineXPCError(code, description: description)
    }
    ...
    private func logFailure(
        ...
        logger.error(
            "VoxOS Refine XPC \(operationName, privacy: .public) failed domain=\(nsError.domain, privacy: .public) code=\(nsError.code, privacy: .public) description=\(nsError.localizedDescription, privacy: .public)"
        )
    }
    ...
    private func connectionInterrupted(
        ...
        cancelIdleShutdown()
        logger.error(
            "VoxOS Refine XPC connection interrupted id=\(identifier.uuidString, privacy: .public)"
        )
    }
