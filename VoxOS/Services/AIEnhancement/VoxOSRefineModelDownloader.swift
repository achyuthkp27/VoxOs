...
struct VoxOSRefineDownloadProgress: Sendable {
    let downloadedBytes: Int64
    let totalBytes: Int64
    let isFinalizing: Bool
}

enum VoxOSRefineDownloadError: LocalizedError {
...
final class VoxOSRefineModelDownloader: @unchecked Sendable {
    ...
    private static let verificationRecordFilename = ".voxos-integrity.json"
    ...
    var progress: VoxOSRefineDownloadProgress {
    ...
    private func performDownload(_ file: ModelFile) async throws {
        ...
        guard let url = downloadURL(for: file) else {
            throw VoxOSRefineDownloadError.invalidDownloadURL(file.path)
        }
        ...
        guard response.statusCode == 200 || response.statusCode == 206 else {
            throw VoxOSRefineDownloadError.unexpectedStatusCode(
                response.statusCode,
                file.path
            )
        }
    ...
    private func streamDownload(
        ...
        delegateQueue.name = "com.achyuthkp.voxos.refine-download"
    ...
    static func validateDownloadedFile(
        ...
        guard actualSize == file.size else {
            throw VoxOSRefineDownloadError.invalidFileSize(
                file.path,
                expected: file.size,
                actual: actualSize
            )
        ...
        guard actualSHA256 == expectedSHA256 else {
            throw VoxOSRefineDownloadError.invalidChecksum(file.path)
        }
    }
    ...
    private final class StreamingDownloadDelegate: NSObject, URLSessionDataDelegate, @unchecked Sendable {
        ...
        func urlSession(
            ...
            redirectedRequest.setValue("VoxOS", forHTTPHeaderField: "User-Agent")
            if let rangeHeader {
        ...
        func urlSession(
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                withState {
                    $0.responseError = VoxOSRefineDownloadError.invalidResponse(
                        file.path
                    )
                }
            ...
            let completion: (() -> Void)? = withState {
                ...
                return {
                    if let responseError {
                        ...
                        continuation.resume(
                            throwing: VoxOSRefineDownloadError.invalidResponse(
                                self.file.path
                            )
                        )
                    }
...
private final class ProgressTracker: @unchecked Sendable {
    ...
    func snapshot() -> VoxOSRefineDownloadProgress {
        lock.lock()
        let downloadedBytes = min(
            totalBytes,
            bytesByFile.values.reduce(Int64(0), +)
        )
        let snapshot = VoxOSRefineDownloadProgress(
            downloadedBytes: downloadedBytes,
