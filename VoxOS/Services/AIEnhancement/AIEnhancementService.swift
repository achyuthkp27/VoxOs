...
class AIEnhancementService: ObservableObject {
    private let logger = Logger(subsystem: "com.achyuthkp.voxos", category: "AIEnhancementService")
    ...
    init(aiService: AIService = AIService(), modelContext: ModelContext) {
        ...
        // Built-in starter prompts (e.g. Agent) are normally seeded during onboarding, but modes
        // referencing them can be created afterward too — make sure they're always present so
        // repairModePromptSelections() below doesn't treat a valid built-in prompt as missing.
        let seedResult = StarterModePromptSeeder.ensurePrompts(for: StarterModeKind.allCases, in: customPrompts)
        var refreshedPrompts = seedResult.didChange ? seedResult.prompts : customPrompts

        // A previously-seeded built-in prompt is a frozen copy — if the code-defined text for one
        // of these ids changes (e.g. a new agent tool argument), keep the user's saved copy in sync
        // rather than silently running stale instructions forever. Users who want a variant should
        // duplicate into a new custom prompt, not hand-edit a built-in one.
        let currentSeedPromptsById = Dictionary(uniqueKeysWithValues: PromptTemplates.seedPrompts.map { ($0.id, $0) })
        var didRefreshBuiltIn = false
        for index in refreshedPrompts.indices {
            guard let seedPrompt = currentSeedPromptsById[refreshedPrompts[index].id],
                refreshedPrompts[index] != seedPrompt
            else { continue }
            refreshedPrompts[index] = seedPrompt
            didRefreshBuiltIn = true
        }

        if seedResult.didChange || didRefreshBuiltIn {
            customPrompts = refreshedPrompts
        }
    ...
    func isConfigured(for configuration: EnhancementRuntimeConfiguration) -> Bool {
        ...
        if provider == .voxOSRefine {
            return aiService.voxOSRefineService.isAvailableInModes
        }
        ...
        do {
                ...
                let aiService = getAIService()
            {
                resultText = await AgentToolExecutor.runLoop(
                    firstReply: requestResult.text,
                    priorMessages: [ChatMessage.user(requestResult.userMessage ?? text)],
                    systemPrompt: requestResult.systemMessage,
    ...
    func repairModePromptSelections() {
        ...
        for index in updatedConfigurations.indices {
            if updatedConfigurations[index].selectedAIProvider == AIProvider.voxOSRefine.rawValue {
                if updatedConfigurations[index].selectedAIModel != VoxOSRefineService.modelName {
                    updatedConfigurations[index].selectedAIModel = VoxOSRefineService.modelName
                    didUpdateModes = true
                }
            }
