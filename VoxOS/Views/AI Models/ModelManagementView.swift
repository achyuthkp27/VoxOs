...
struct ModelManagementView: View {
    ...
    @ObservedObject private var voxOSRefineService = VoxOSRefineService.shared
    ...
    private var localModelsSection: some View {
        VStack(spacing: 12) {
            VoxOSRefineModelCardView(
                service: voxOSRefineService,
                deleteAction: confirmDeleteVoxOSRefineModel
            )
    ...
    private var importLocalModelButton: some View {
        HStack(spacing: 8) {
            ...
            InfoTip(
                "Add a custom fine-tuned whisper model to use with VoxOS. Select the downloaded .bin file.",
    ...
    private func confirmDeleteVoxOSRefineModel() {
        alertTitle = String(localized: "Delete VoxOS Refine?")
        alertMessage = String(
            localized: "The model will need to be downloaded again before a Mode can use it."
        )
        deleteActionClosure = {
            Task {
                await voxOSRefineService.deleteModel()
            }
        }
        isShowingDeleteAlert = true
    }
